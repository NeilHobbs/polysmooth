##Micromosaics of  Mixture LLIN and Standard LLIN
...
#get the standard llin only simulations:: ["Lower Benchmark"]
...
overall.plot = ggplot(final.df,
  ...
  scale_fill_manual(values = c("darkblue",
                               "darkred",
                               "black"))+
  xlab("Proportion Coverage of Mixture LLIN")+
  ylab("Count")+
  facet_grid(dose *comparator ~ initial.pyr.resistance)+
  ggtitle(paste0("Total Overall Change\nInitial Pyrethroid Bioassay Survival (%)"))+
  theme_bw()+
...
pyr.plot = ggplot(final.df,
  ...
  xlab("Proportion Coverage of Mixture LLIN")+
  ylab("Count")+
  ggtitle(paste0("Pyrethroid Insecticide Change\nInitial Pyrethroid Bioassay Survival (%)"))+  facet_grid(dose *comparator ~ initial.pyr.resistance)+
  theme_bw()+
  theme(legend.position = "none")
...
novel.plot = ggplot(final.df,
  ...
  scale_fill_manual(values = c("darkblue",
   ...
   xlab("Proportion Coverage of Mixture LLIN")+
  ylab("Count")+
  ggtitle(paste0("Novel Insecticide Change\nInitial Pyrethroid Bioassay Survival (%)"))+  facet_grid(dose *comparator ~ initial.pyr.resistance)+
  theme_bw()+
  theme(legend.position = "none")

plot.legend = cowplot::get_legend(ggplot(final.df,
                                         aes(x=coverage,
                                             y=novel.freq,
                                             fill = outcome.overall))+
                                    geom_col(colour = "grey")+
...
pyr.plot + novel.plot + overall.plot + plot.legend + plot_layout(design = the.layout)
...
plot_comparison_vs_mixtures = function(the.df,
    ...
    A =  subset(the.df,
                coverage.mixture != 0 & #removes just LLIN only if there
    ...
    #This is how  micro-mosaic compared to mixture LLIN only
    plot.pyr = ggplot(A, aes(
      ...
      scale_fill_manual(values = c("blue", "red", "black"),
                        drop = FALSE)+
      scale_y_continuous(sec.axis = sec_axis(~ . , name = "Proportional Coverage Mixture LLIN",
                                             breaks = NULL,
                                             labels = NULL))+
...
plot_comparison_vs_standard = function(the.df,
  ...
  A =  subset(the.df,
              coverage.mixture != 0 & #removes just LLIN only
  ...
  #This is how the micro-mosaic performed against standard LLIN only:
  plot.pyr = ggplot(A, aes(
    ...
    scale_fill_manual(values = c("blue", "red", "black"),
                      drop = FALSE)+
    xlab(paste0(" Change in Pyrethroid\nBioassay Survival"))+
    scale_y_continuous(sec.axis = sec_axis(~ . , name = "Proportional Coverage Mixture LLINs",
                                           breaks = NULL,
                                           labels = NULL))+
    scale_x_continuous(sec.axis = sec_axis(~ . , name = paste0("Initial Pyrethroid\nBioassay Survival (%)"),
                                           breaks = NULL,
    ...
    theme(axis.text.x = element_text(size = 6),
          legend.position = "none")
  #Where negative values --> more resistance in LLIN only
  ...
  the.plot = plot.pyr +
    ...
    plot.overall +
    plot_annotation(title =
                      paste0("Standard-Mixture Micro-Mosaics vs Standard LLINs only:", mixture.type)
    )
