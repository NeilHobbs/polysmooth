...
choose_next_mixture = function(previous.mixture,
  ...
  if(candidate.mixture %in% available.mixtures){return(candidate.mixture)}else(
  ...
  while(candidate.mixture %notin% available.mixtures){
    ...
    else(candidate.mixture = (candidate.mixture + 1))
         if(candidate.mixture %in% available.mixtures){return(candidate.mixture)}#try to go to the next available insecticide
