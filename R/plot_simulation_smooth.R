...
plot_simulation_smooth = function(simulation.dataframe, #created from the get_simulation_dataframe() function
  ...
  simulation.dataframe$bioassay.survival = convert_resistance_score_to_bioassay_survival(maximum.bioassay.survival.proportion = 1,
                                                                                          trait.mean = simulation.dataframe$resistance.score,
                                                                                          michaelis.menten.slope = 1,
                                                                                          half.population.bioassay.survival.resistance = half.population.bioassay.survival.resistance) * 100#As a percentage)
  #Make as separate dataframes: 1 for intervention, 1 for refugia
  temp.df.intervention = simulation.dataframe%>%
    dplyr::filter(site == "intervention")

  temp.df.refugia = simulation.dataframe%>%
    dplyr::filter(site == "refugia")

  #Create a plot for the intervention site: this will have the deployment sequence and the threshold lines.
  intervention.plot = ggplot(data = temp.df.intervention, aes(x=time.in.generations,
                                                        y = bioassay.survival,
    ...
    geom_line(data =temp.df.intervention, aes(x=time.in.generations,
  ...
  return(gridExtra::grid.arrange(intervention.plot, refugia.plot, ncol=2))
}
