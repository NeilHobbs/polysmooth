...
convert_bioassay_survival_to_resistance_score = function(maximum.bioassay.survival.proportion = 1, #must be set to 1 to work properly
  ...
  while((test.population.resistance = ((minimum.resistance.value + maximum.resistance.value)/2))){
    ...
    {return(test.population.resistance)} #When precision level reached return population resistance
      ...
      if(convert_resistance_score_to_bioassay_survival(
        ...
        trait.mean = test.population.resistance,
        michaelis.menten.slope = michaelis.menten.slope,
        half.population.bioassay.survival.resistance = half.population.bioassay.survival.resistance) < bioassay.survival) #check if survival less than bioassay survival
      {
