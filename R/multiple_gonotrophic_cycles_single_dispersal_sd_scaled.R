multiple_gonotrophic_cycles_single_dispersal_sd_scaled = function(coverage = 0.8,
                                                                  natural.survival = 1,
                                                                  dispersal.rate = 0.2,
                                                                  female.exposure = 0.7,
  ...
  calculate.female.fitness.cost.refugia  = female.fitness.cost.refugia * (sd_changes_with_z(current.z = refugia.trait.mean,
                                                                                            z.sd.intercept = z.sd.intercept,
                                                                                            z.sd.coefficient = z.sd.coefficient))

  calculate.female.fitness.cost.intervention = female.fitness.cost.intervention* (sd_changes_with_z(current.z = intervention.trait.mean,
                                                                                                              z.sd.intercept = z.sd.intercept,
                                                                                                              z.sd.coefficient = z.sd.coefficient))
  ...
  normal.distribution.trait.1.intervention = create_normal_distribution(vector.length = vector.length,
                                                                        ...
                                                                        standard.deviation = sd_changes_with_z(current.z = intervention.trait.mean,
                                                                                                               z.sd.intercept = z.sd.intercept,
                                                                                                               z.sd.coefficient = z.sd.coefficient))

  normal.distribution.trait.1.refugia = create_normal_distribution(vector.length = vector.length,
                                                                   trait.mean = refugia.trait.mean,
                                                                   standard.deviation = sd_changes_with_z(current.z = refugia.trait.mean,
                                                                                                          z.sd.intercept = z.sd.intercept,
                                                                                                          z.sd.coefficient = z.sd.coefficient))
  ...
  for(gonotrophic in 1:n.cycles){
    ...
    if(gonotrophic == 1){
      ...
      intervention.selection.differential.a = (exposure.scaling.factor*(new.intervention.mean.a - intervention.trait.mean)) - calculate.female.fitness.cost.intervention

      refugia.selection.differential.b = (exposure.scaling.factor*(new.refugia.mean.b - refugia.trait.mean)) - calculate.female.fitness.cost.refugia
    ...
    if(gonotrophic != 1){
      ...
      #Calculate the selection differentials::
      intervention.selection.differential.a = (exposure.scaling.factor *((new.intervention.mean.a - intervention.trait.mean))) - calculate.female.fitness.cost.intervention
      intervention.selection.differential.b = (exposure.scaling.factor *(new.intervention.mean.b - refugia.trait.mean)) - calculate.female.fitness.cost.refugia

      refugia.selection.differential.a = (exposure.scaling.factor *(new.intervention.mean.a - intervention.trait.mean)) - calculate.female.fitness.cost.intervention
      refugia.selection.differential.b = (exposure.scaling.factor *(new.refugia.mean.b - refugia.trait.mean)) - calculate.female.fitness.cost.refugia
