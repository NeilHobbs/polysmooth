...
irm_strategy_sequence_mixture_decay = function(number.of.insecticides,
                                         current.generation,
  ...
  list.available.withdrawn =  return_and_withdrawal_of_insecticides_from_arsenal(number.of.insecticides,
                                                                    current.generation,
                                                                    withdrawal.threshold,
                                                                    return.threshold,
                                                                    simulation.array,
  ...
  if(nrow(available.mixtures.df)==0){deployment.df.updated = deploy_mixture_with_decay(candidate.mixture.id = NA,
    ...
    if(current.mixture %in% available.mixtures.df$mixture.id){deployment.df.updated = deploy_mixture_with_decay(candidate.mixture.id = current.mixture,
                                                                                                     mixture.df = mixture.df,
                                                                                                     deployment.df = deployment.df,
