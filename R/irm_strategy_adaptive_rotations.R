#'@title Implement the rotation insectidicide resistance management strategy; with sequence when only 1 insecticide available
#'
...
irm_strategy_adaptive_rotations = function(number.of.insecticides,
                                                          current.generation,
  ...
  if(length(available.to.deploy)==0){deployment.vector.updated = deploy_the_chosen_insecticide(insecticide.to.deploy = NA,
    ...
    if(candidate.insecticide == current.insecticide){
      deployment.vector.updated = deploy_the_chosen_insecticide(insecticide.to.deploy = candidate.insecticide,
