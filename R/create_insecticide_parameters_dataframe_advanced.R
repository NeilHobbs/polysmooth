...
create_insecticide_parameters_dataframe_advanced = function(number.of.insecticides,
  ...
  if(length(female.fitness.cost) == number.of.insecticides){
    female.fitness.cost = female.fitness.cost}else{if(length(female.fitness.cost) == 1){
      female.fitness.cost = rep(female.fitness.cost, times=number.of.insecticides)}else{
        stop("heritability must either be a vector with length equal to the number.of.insecticides or a single value")
      }}
