wrapper_run_simulation_micromosaics_sd_scaled = function(insecticide.parameters.df,
                                                         maximum.generations,
                                                         sim.array,
                                                         z.sd.intercept,
                                                         z.sd.coefficient,
  ...
  for(generation in 2:maximum.generations){
    for(insecticide in 1:number.of.insecticides){
      if(insecticide == deployed.vector.1[generation] |
        ...
        tracked.resistance = multiple_gonotrophic_cycles_micromosaic_dispersal_sd_scaled(intervention.trait.mean.i = sim.array['intervention', insecticide, generation-1],
                                                                                         ...
                                                                                         male.differential.intervention.j = calculate_male_insecticide_fitness_selection_differential(male.insecticide.selection.differential = male.selection.differentials[[2]],
                                                                                                                                                                                      exposure.scaling.factor = exposure.scaling.factor,
                                                                                                                                                                                      male.fitness.selection.differential = (sd_changes_with_z(current.z = sim.array['intervention', other.insecticide, generation-1],
                                                                                                                                                                                                                                               z.sd.intercept = z.sd.intercept,
                                                                                         ...
                                                                                         male.differential.refugia.j = wrapper_male_fitness_selection_differential(male.trait.mean = sim.array['refugia', other.insecticide, generation-1],
                                                                                                                                                                   male.fitness.cost = (sd_changes_with_z(current.z = sim.array['refugia', other.insecticide, generation-1],
                                                                                                                                                                                                          z.sd.intercept = z.sd.intercept,
                                                                                                                                                                                                          z.sd.coefficient = z.sd.coefficient) * insecticide.parameters.df$male.fitness.cost[other.insecticide])),
      ...
      male.insecticide.selection.differentials = perform_male_micromosaic_smooth_sd_scaled(insecticide.coverage.1 = tracked.coverage,
                                                  insecticide.coverage.2 = other.coverage,
                                                  trait.mean.1 = sim.array["intervention", deployed.vector.1[generation], generation-1],
                                                  trait.mean.2 = sim.array["intervention", deployed.vector.2[generation], generation-1],
                                                  z.sd.coefficient = z.sd.coefficient,
                                                  z.sd.intercept = z.sd.intercept,
                                                  vector.length = vector.length,
                                                  female.exposure = female.exposure,
      ...
      tracked.resistance = multiple_gonotrophic_cycles_micromosaics_dispersal_not_deployed_sd_scaled(intervention.trait.mean.i = sim.array['intervention', deployed.vector.1[generation], generation-1],
                                                                                                     intervention.trait.mean.j = sim.array['intervention', deployed.vector.1[generation], generation-1],
                                                                                                     ...
                                                                                                     coverage = coverage,
                                                                                                     dispersal.rate = dispersal.rate,
                                                                                                     male.differential.intervention.i = calculate_male_insecticide_fitness_selection_differential(male.insecticide.selection.differential = male.insecticide.selection.differentials[[1]],
                                                                                                                                                                                                  exposure.scaling.factor = exposure.scaling.factor,
                                                                                                                                                                                                  male.fitness.selection.differential = (sd_changes_with_z(current.z = sim.array['intervention', deployed.vector.1[generation], generation-1],
                                                                                                                                                                                                                                                           z.sd.intercept = z.sd.intercept,
                                                                                                                                                                                                                                                           z.sd.coefficient = z.sd.coefficient) * insecticide.parameters.df$male.fitness.cost[deployed.vector.1[generation]])),
                                                                                                     male.differential.intervention.j = calculate_male_insecticide_fitness_selection_differential(male.insecticide.selection.differential = male.insecticide.selection.differentials[[1]],
                                                                                                                                                                                                  exposure.scaling.factor = exposure.scaling.factor,
                                                                                                                                                                                                  male.fitness.selection.differential = (sd_changes_with_z(current.z = sim.array['intervention', deployed.vector.2[generation], generation-1],
                                                                                                                                                                                                                                                           z.sd.intercept = z.sd.intercept,
                                                                                                                                                                                                                                                           z.sd.coefficient = z.sd.coefficient) * insecticide.parameters.df$male.fitness.cost[deployed.vector.2[generation]])),
                                                                                                     ...
                                                                                                     male.differential.refugia.i = wrapper_male_fitness_selection_differential(male.trait.mean = sim.array['refugia', deployed.vector.1[generation], generation-1],
                                                                                                                                                                               male.fitness.cost = (sd_changes_with_z(current.z = sim.array['refugia', deployed.vector.1[generation], generation-1],
                                                                                                                                                                                                                      z.sd.intercept = z.sd.intercept,
                                                                                                                                                                                                                      z.sd.coefficient = z.sd.coefficient) * insecticide.parameters.df$male.fitness.cost[deployed.vector.1[generation]])),
                                                                                                     male.differential.refugia.j = wrapper_male_fitness_selection_differential(male.trait.mean = sim.array['refugia', deployed.vector.2[generation], generation-1],
                                                                                                                                                                               male.fitness.cost = (sd_changes_with_z(current.z = sim.array['refugia', deployed.vector.2[generation], generation-1],
                                                                                                                                                                                                                      z.sd.intercept = z.sd.intercept,
                                                                                                                                                                                                                      z.sd.coefficient = z.sd.coefficient) * insecticide.parameters.df$male.fitness.cost[deployed.vector.2[generation]])),
                                                                                                     male.differential.refugia.tracked = wrapper_male_fitness_selection_differential(male.trait.mean = sim.array['refugia', insecticide, generation-1],
                                                                                                                                                                                     male.fitness.cost = (sd_changes_with_z(current.z = sim.array['refugia', insecticide, generation-1],
                                                                                                                                                                                                                            z.sd.intercept = z.sd.intercept,
                                                                                                                                                                                                                            z.sd.coefficient = z.sd.coefficient) * insecticide.parameters.df$male.fitness.cost[insecticide])),
                                                                                                     female.fitness.cost.i = insecticide.parameters.df$male.fitness.cost[deployed.vector.1[generation]],
                                                                                                     female.fitness.cost.j = insecticide.parameters.df$male.fitness.cost[deployed.vector.2[generation]],
                                                                                                     female.fitness.cost.tracked = insecticide.parameters.df$male.fitness.cost[insecticide],
