...
deploy_mixture_with_decay = function(candidate.mixture.id,
  ...
  if(is.na(candidate.mixture.id)){
    ...
    mixture.id =  rep(NA, times = 1)
    mixture.part.1 = rep(NA, times = 1)
    mixture.part.2 = rep(NA, times = 1)

    part.1 = mixture.part.1[1]
    part.2 = mixture.part.2[1]

    insecticide.efficacy.vector.part.1 = rep(NA, times = 1)

    insecticide.efficacy.vector.part.2 = rep(NA, times = 1)
