##Rotations simulations to compare against micromosaics
library(devtools)
load_all()
parameter.space.micromosaics = read.csv("Simulation Experiments/Setting up Simulations/parameter.space.micromosaics.csv")

parameter.space.micromosaics = do.call("rbind", replicate(9, parameter.space.micromosaics, simplify = FALSE))

parameter.space.micromosaics$cross.resistance = rep(c(-0.3, 0, 0.3), each = 15000)

parameter.space.micromosaics$decay.rate.i = rep(rep(c(0.005,
                                                      0.015,
                                                      0.025), each = 5000), 3)

parameter.space.micromosaics$decay.rate.j = rep(0.015, 45000) #is fixed and can just compare vs higher low

temp.list = list()

for(i in 1:45000){
  A = subset(convert_output_to_dataframe_singles(wrapper_run_simulation_singles(insecticide.parameters.df = create_insecticide_parameters_dataframe_advanced(number.of.insecticides = 2,
                                                                                                                                                             applied.insecticide.dose = 1,
                                                                                                                                                             recommended.insecticide.dose = 1,
                                                                                                                                                             threshold.generation = 20,
                                                                                                                                                             base.efficacy.decay.rate = c(parameter.space.micromosaics$decay.rate.i[i],
                                                                                                                                                                                          parameter.space.micromosaics$decay.rate.j[i]),
...
temp.df$strategy = "rotations"

write.csv(temp.df, "mm_simulations_rotation_comparator_decay.csv")
