...
df = read.csv(".//polysmooth_calibration.csv")

df.1 = df%>%
  dplyr::filter(exposure.scaling.factor == 10)
...
ggplot(df.3, aes(x=sim.duration/10))+
  ...
  geom_vline(xintercept = 12,
             ...
             alpha = 0.7,
             linetype = "dashed")+
  geom_rect(data = subset(df.3, exposure.scaling.factor %in% 10 & standard.deviation %in% 50),
...
ggplot(df.1, aes(x=sim.duration))+
  geom_histogram()+
...
df.2 = df.1%>%
  dplyr::filter(standard.deviation == 40|
