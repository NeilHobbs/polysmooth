...
p.1 = ggplot(mm.single.g, aes(x=time.in.generations,
  ...
  geom_text(aes(x=380, y=5, label = paste0("Rotations\nperform\nbetter than\nmicro-mosaics")),
                size = 5.5)+
...
p.2 = ggplot(mm.multi.g, aes(x=time.in.generations,
  ...
  geom_hline(yintercept = 10, colour = "grey", linetype = "dotted",
             size = 1.3)+
  geom_text(aes(x=350, y=5, label = paste0("Rotations\nperform\nmarginally\nbetter than\nmicro-mosaics")),
                size = 5.5)+
  theme_bw()+
  xlab("Time in Generations")+
  ylab("Bioassay Survival (%)")+
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, 27),
                     breaks = seq(0, 25, 5))+
  ggtitle(paste0("Five Gonotrophic Cycles:\n No Natural Mortality"))+
  theme(axis.title.x = element_text(size = 12),
        axis.title.y = element_text(size = 12),
        axis.text.x = element_text(size = 10),
        axis.text.y = element_text(size = 10))
...
p.3 = ggplot(mm.multi.g.nat.mort, aes(x=time.in.generations,
  ...
  geom_hline(yintercept = 10, colour = "grey", linetype = "dotted",
             size = 1.3)+
  geom_text(aes(x=380, y=5, label = paste0("Micro-mosaics\nperform\nbetter than\nrotations")),
                size = 5.5)+
  theme_bw()+
  xlab("Time in Generations")+
  ylab("Bioassay Survival (%)")+
